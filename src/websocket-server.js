...
async function loadAuthorizedTokens() {
  try {
    ...
    console.error(`Loaded ${authorizedTokens.size} authorized channel-token pairs from ${TOKENS_FILE}`);
    return true;
  } catch (error) {
  ...
  if (clientChannel === REGISTER_PATH) {
    ...
    ws.once('message', async (message) => {
      ...
      try {
        ...
        const channelPath = `/${host.replace(':', '_')}`;
...
const parseArgs = () => {
  ...
  return { port, quit, newToken, encodedPair, daemon };
};
...
const main = async () => {
  ...
  if (config.newToken) {
    ...
    const serverAddress = `ws://localhost:${config.port}`;
    ...
    if (!serverStatus.running) {
      console.log('Note: Server is not currently running. Start it with: node websocket-server.js');
      process.exit(0);
    } else {
  ...
  httpServer.listen(PORT, () => {
    ...
    console.log(`Server has ${authorizedTokens.size} authorized channels`);
    console.log(`WebMCP client token (for MCP path): ${serverToken}`);
    console.log(`WebMCP client URL: ws://localhost:${PORT}${MCP_PATH}?token=${serverToken}`);
    console.log(`Use 'node websocket-server.js --new <encoded-pair>' to authorize a channel-token pair`);
  });
